...
- **Library Row Vertical Kebab More Actions Button (`⋮`) (`BrowserRowMoreButton.kt`, `PresetListPanel.kt`, `PlaylistEditorPanel.kt`, `QueueActionsPanel.kt`, `BgQueueActionsPanel.kt`)**: Added a right-aligned vertical kebab button (`Icons.MORE_VERTICAL`) to rows across all four Library panels (Presets List, Playlist Editor items, A/B Play Queue, and Background Queue). Appears automatically on mouse hover, when an item is selected via keyboard arrow keys or mouse click, or while its context menu is open. Left-clicking the button or right-clicking anywhere on the row opens the full contextual action menu (Deck routing, queue assignment, playlist inclusion, renaming, duplicating, and deleting).
