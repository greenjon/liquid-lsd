...
- **Enhanced LFO analytical step resolution**: The number of analytical sampling steps for each half of the LFO scope is now derived with a higher 1.5x pixel-width multiplier (`(pixelWidth * 1.5f).toInt().coerceIn(60, 1000)`), raised from `0.5f` and capped at 1000 steps (up from 300). This eliminates polygonal stepping and pixelation on fast LFO periods (< 5s) while keeping rendering zero-allocation.
