...
> [!NOTE]
> **Release 1.0.0-beta.27** introduces a multi-scale oscilloscope engine with calibrated physical timebases ($1\text{s}$ to $24\text{h}$), real-time future waveform projection for deterministic LFOs, an interactive "NOW" playhead with history/future ratios, and calibrated grid divisions with dynamic timestamp labels.
...
- **Oscilloscope & Modulation Performance Optimization**: Replaced per-call `HashSet` instantiation in `isCvSourceBipolar` with a zero-allocation branch, eliminating over $180{,}000$ GC object allocations per second on the 60 FPS render path during anti-aliased waveform rendering.
- **Render-Loop Hot-Path Cleanups**: Preallocated immutable timebase lists/arrays in `OscilloscopeDrawer` and reused persistent `ImInt` wrappers across oscilloscope timebase combos and `ModulatorHeaderRow` operator selectors, ensuring compliance with strict ImGui zero-allocation draw rules.
- **Eliminated Dead Multi-Trace History Loops**: Removed unused per-frame modulator history evaluation loops and unreferenced `modulatorHistories` buffers in `FinalParamSection` and `CellConfigPanel`, freeing CPU cycles for the rendering pipeline.
