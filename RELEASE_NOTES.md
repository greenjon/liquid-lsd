...
#### 0.2. Universal 3D Tri-Axial Display Mode (`Deck.kt`, `Renderer.kt`, `PresetGridTabs.kt`, `PresetModels.kt`, `tri_planar.vert`, `tri_planar.frag`)
...
- **Perspective-Correct Interpolation & Square Plane Aspect**: Rendered 2D sources into a square $1:1$ `rawSourceFBO` (`FBO(height, height)`) to eliminate non-uniform plane stretching, and emitted homogeneous clip coordinates with $w$-buffering in `tri_planar.vert` for hardware perspective-correct texture mapping without affine distortion.
...
- **Full DynamicVisualSource Unification**: Removed special-case `if (source is Mandala)` / `if (mandala != null)` branching across UI layout, preset serializer, and preset model layers. Mandala is now handled identically to all other dynamic visual generator engines.
- **View & SRC Tab Decomposition**: Reordered Mandala's `meta.json` declaration so generator geometry controls (`Lobes`, `Recipe Select`, `L1`–`L4`, `Thickness`, `Hue Offset`, `Hue Sweep`, `Depth`) populate the `SRC` tab in natural declaration order, while spatial transform controls (`Zoom`, `Rotate Z`, `Rotate Y`, `Rotate X`) automatically route to the generic `View` subtab via `TRANSFORM_PARAM_NAMES`.
