...
- **Universal 2D View Transformation (`view2d.frag`)**: When in standard 2D mode (`3D Mode` = 0), universal View parameters `Zoom` and `Rotate Z` apply smoothly to any 2D visual source with aspect-ratio-corrected rotation and transparent border handling via `rawSource2DFBO`.
