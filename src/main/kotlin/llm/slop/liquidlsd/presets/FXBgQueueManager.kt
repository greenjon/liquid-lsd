...
object FXBgQueueManager {
    ...
    fun appendToQueue(file: File) {
        if (file.extension.equals("lsdfxplay", ignoreCase = true)) {
            ...
            logger.info { "Appended FX playlist to BG FX queue: ${file.name} (${files.size} items)" }
        } else {
            val insertPos = queue.size
            queue.add(file)
            ...
            logger.info { "Appended FX item to BG FX queue: ${file.name}" }
        }
    }
    ...
    fun insertAt(index: Int, files: List<File>) {
        ...
        logger.info { "Inserted ${files.size} FX item(s) at index $clampedIndex into BG FX queue" }
    }
    ...
    fun removeFromQueue(index: Int) {
        ...
        logger.info { "Removed FX item from BG FX queue at index $index: ${removed.name}" }
    }
    ...
    fun moveItem(fromIndex: Int, toIndex: Int) {
        ...
        logger.info { "Moved BG FX queue item from $fromIndex to $toIndex: ${item.name}" }
    }
    ...
    fun clearQueue() {
        ...
        playedIndices.clear()
        playbackHistory.clear()
        logger.info { "Cleared BG FX queue" }
    }
    ...
    fun jumpToIndex(index: Int, session: SessionContext, mixer: Mixer) {
        ...
        logger.info { "Jumping to BG FX queue item at index $activeIndex: ${file.name} on Deck BG" }
        FXItemApplier.apply(session, file, mixer.deckBG)
    }
    ...
    fun advanceNext(session: SessionContext, mixer: Mixer) {
        ...
        if (isShuffleEnabled) {
            ...
            if (unplayed.isEmpty()) {
                if (isRepeatEnabled) {
                    ...
                    logger.info { "End of shuffle BG FX queue reached (all items played once)." }
                    return
                }
            } else {
            ...
            if (nextIndex >= queue.size) {
                if (isRepeatEnabled) {
                    ...
                    logger.info { "End of BG FX queue reached." }
                    return
                }
            }
        }
        ...
        val file = queue[activeIndex]
        logger.info { "Advancing BG FX queue to index $activeIndex: ${file.name} on Deck BG" }
        FXItemApplier.apply(session, file, mixer.deckBG)
    }
    ...
    fun advancePrevious(session: SessionContext, mixer: Mixer) {
        ...
        if (isShuffleEnabled) {
            if (playbackHistory.isNotEmpty()) {
                ...
                logger.info { "No previous item in BG FX queue shuffle history." }
                return
            }
        } else {
            ...
            if (prevIndex < 0) {
                if (isRepeatEnabled) {
                    ...
                    logger.info { "Beginning of BG FX queue reached." }
                    return
                }
            }
        }
        ...
        val file = queue[activeIndex]
        logger.info { "Stepping back BG FX queue to index $activeIndex: ${file.name} on Deck BG" }
        FXItemApplier.apply(session, file, mixer.deckBG)
    }
}
