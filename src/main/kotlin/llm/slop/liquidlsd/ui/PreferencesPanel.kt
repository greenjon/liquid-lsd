...
import llm.slop.liquidlsd.midi.MidiControlMapping
...
object PreferencesPanel {
    ...
    fun draw(session: llm.slop.liquidlsd.SessionContext, currentSize: Float = session.uiTheme.baseSize, displayW: Float, displayH: Float,
        ...
        if (ImGui.beginChild("##preferences_content", rightContentW, contentH, true)) {
            when (activeCategory) {
                ...
                Category.MIDI_CONTROLLER  -> drawMidiPreferences(session, mixer, parametersState)
    ...
    private fun drawMidiPreferences(session: llm.slop.liquidlsd.SessionContext, mixer: llm.slop.liquidlsd.rendering.Mixer?, parametersState: ParametersState?) {
