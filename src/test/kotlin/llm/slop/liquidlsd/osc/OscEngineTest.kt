...
class OscEngineTest {
    ...
    fun testReceivesAndQueuesInboundMessageOnLoopback() {
        ...
        waitUntil { OscEngine.boundInboundPort() > 0 }
        val port = OscEngine.boundInboundPort()

        val payload = OscCodec.encode(OscMessage("/1/fader1", listOf(0.42f)))
        DatagramSocket().use { socket ->
            socket.send(DatagramPacket(payload, payload.size, InetAddress.getLoopbackAddress(), port))
        }

        waitUntil { OscEngine.inboundQueue.isNotEmpty() }
    ...
    fun testAutoLearnsRemoteClientAddressFromInboundSender() {
        ...
        waitUntil { OscEngine.remoteAddressLabel() != "(none)" }
    ...
    fun testPacketSnifferRecordsInboundTraffic() {
        ...
        waitUntil { OscEngine.getRecentPackets().any { it.address == "/sniff/test" } }
    ...
    fun testStopClearsQueueAndSnifferState() {
        ...
        waitUntil { OscEngine.inboundQueue.isNotEmpty() && OscEngine.getRecentPackets().isNotEmpty() }
