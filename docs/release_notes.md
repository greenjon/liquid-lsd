...
- **Preset Grid Horizontal Column Headers & Split Tab Layout**: Converted column titles into horizontal headers (`VAL`, `MIDI`, `LFO`, `AUD`, `TRIG`) sized at `UITheme.FontLevel.BODY`, retained the video source dropdown button (`[Source ▾]`) in the title bar beside `"Preset Grid"` with 24px spacing, repositioned the `[FX]` and `[View]` section tabs above the first parameter name with a 24px left indent, and aligned the top of the left side tabs (`MIX`, `A`, `B`, `BG`, `PV`) with the second parameter row.
