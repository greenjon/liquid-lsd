...
- **Preset Grid Horizontal Column Headers & Title Bar Subtabs**: Converted column titles into horizontal headers (`VAL`, `MIDI`, `LFO`, `AUD`, `TRIG`) sized at `UITheme.FontLevel.BODY`, integrated the deck subtab row (`[Source ▾]`, `[FX]`, `[View]`) directly into the panel title bar beside `"Preset Grid"`, and aligned the top of the left side tabs (`MIX`, `A`, `B`, `BG`, `PV`) with the second parameter row.
