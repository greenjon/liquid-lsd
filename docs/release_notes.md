...
- **Preset Grid Horizontal Column Headers & Tab Alignment**: Converted column titles from vertical multi-line stacks into horizontal headers (`VAL`, `MIDI`, `LFO`, `AUD`, `TRIG`) sized at `UITheme.FontLevel.BODY`, left-aligned the deck subtab row (`[Source ▾]`, `[FX]`, `[View]`) to share the exact same left-hand origin as the side tabs, and aligned the top of the left side tabs (`MIX`, `A`, `B`, `BG`, `PV`) with the second parameter row.
