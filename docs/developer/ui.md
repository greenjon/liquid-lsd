...
- **Performance Matrix row sizing (`PerformanceMatrixPanel.drawMatrix`)**: row height is `availH / rows.size`, floored at `MIN_ROW_H` (75px) so all four rows fit simultaneously when the Library is opened to its default half-height view. Top header bar consolidates the tab strip (`[DECKS]` / `[MASTER]`), rack controls (`[Close Edit]`), persistent Learn indicator, and `[ALL]` dice into a single compact 26px line. Row titles are rendered inline on the left of their controls (Deck rows feature a styled Eurorack-style `[DECK / A]` badge beside the two stacked toolbar rows; Master, Transitions, and Clock rows inline their titles on the left of their header bars), eliminating vertical title-above-control stacking and reducing row height overhead. Knobs dynamically scale between 24px and 100px (typically ~38-44px at compact half-view heights). Wing controls use `ctrlH` (21px in compact mode, 24px otherwise). Click-drag scrolling comes from a grid-wide `##perf_grid_drag_scroll` invisible button submitted first with `setNextItemAllowOverlap()`, plus the title-band drop-zone buttons; each calls `applyDragScroll()`, which scrolls the child by `io.mouseDelta.y` while that item is active. The background button also stops an empty-space drag from moving the host window. The knob body blends toward its accent color on hover (22%) and while dragging (38%).
