...
graph TD
    ...
    UIManager --> PerformanceMatrixPanel[PerformanceMatrixPanel.kt - Performance Mode 4x4 macro matrix]
...
- **Workspace Modes (`UITheme.WorkspaceMode`)**: The top-level workspace is either `WorkspaceMode.CLASSIC` (the three-column Parameters/Properties/Mixer layout documented above) or `WorkspaceMode.RACK` (retained enum name for save-file compatibility; it now drives **Performance Mode**, the `PerformanceMatrixPanel.kt` 4×4 macro knob matrix — see `docs/user_guide/macros_and_rack.md`). Performance Mode is the default view mode for new users (`AppPreferences.workspaceMode = RACK`), and the user's active choice is persisted via `AppPreferencesStore` to `lsd-preferences.properties` across shutdown and startup. `UIManager.render()` branches on `session.uiTheme.workspaceMode` to draw one or the other; in Performance Mode, `PerformanceMatrixPanel` replaces Columns 1 & 2 while the Mixer and Library dock remain visible. `F4` is wired in `UIManager.processQueueKeyboardShortcuts()` as a live two-way toggle (`ImGui.isKeyPressed(ImGuiKey.F4)` flips `RACK` ↔ `CLASSIC` regardless of which mode is active). The `View` menu (`MenuBar.kt`) additionally lists direct-select items "Classic Deck View" and "Performance Mode (F4)" — clicking either selects that mode directly. The earlier 19" Modular Video Rack chassis UI (faceplates, rear patch-cable view, `Tab` flip) has been deleted entirely; see `docs/developer/modular_video_rack_proposal.md` for its retired history.
