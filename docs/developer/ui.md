...
- **Settings Category Routing**: `SettingsPanel` organizes application preferences into 7 clean categories (`APPEARANCE`, `VIDEO_DISPLAY`, `AUDIO_ENGINE`, `BROADCAST`, `MIDI_CONTROL`, `SHORTCUTS`, `GENERAL`) and supports targeted opening via `SettingsPanel.open(category)`. The Preset Grid layout ratio (`gridCellRatio`) is consolidated inside `APPEARANCE` under "Fonts & Sizing".
