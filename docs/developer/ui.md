...
graph TD
    ...
    PerformanceMatrixPanel --> PerformanceTransitionsControls[PerformanceTransitionsControls.kt - Transitions row line]
...
- **Single workspace (Performance Mode)**: `PerformanceMatrixPanel.kt` (4×4 macro knob matrix + Deep Edit — see `docs/user_guide/macros_and_rack.md`) is the primary live editing view, orchestrating dedicated sub-controllers for clean separation of concerns:
  ...
  - `PerformanceMasterControls.kt`: Master row's deck-style stacked left wing: `[MIX]` pill + crossfader line (`drawCrossfader`: deck snap badges, crossfader track taking the leftover width, AUTO, fade-duration badge) over `[FX]` pill + Master FX `FxChainHeader` and right-wing Master FX bypass. The row's mode is `PerformanceUiContext.isMasterRowFx` (`masterRowMode == "FX"` or Deep Edit Mixer sub-tab `FX`), mirroring the deck rows' `deckRowMode`/deck sub-tab rule; in FX mode the row's bankId is substituted to `MASTER_FX`; the badge still reads `MASTER` and the crossfader stays on the `[MIX]` line.
  - `PerformanceDeckControls.kt`: Deck rows left/right wing controls (stacked two-row left wing: Row 1 SRC pill, generator badge, preset combo, eject, randomize, queue navigation; Row 2 FX pill, FX chain header controls; right wing FX bypass).
...
- **Performance Matrix row sizing (`PerformanceMatrixPanel.drawMatrix`)**: row height (`baseRowH`, computed in `draw`) is the Perform-view grid height divided by the tab's row count, floored at `MIN_ROW_H` (68px) so all four rows fit above a HALF Library in a 1280x720 display's ~688px window (the PerformanceMatrix window also uses 4px vertical padding for this). In Edit view `UIManager` passes `hiddenLibraryH` (the HALF Library height that isn't drawn), so `baseRowH` and the knob diameter still come from the Perform layout of the tab holding the open row (`layoutTabIdx`/`layoutRows`); the open row is `baseRowH + expandedExtraH` (caption line + Learn button) tall, with its controls and knobs laid out in the top `baseRowH` band so nothing moves when Deep Edit opens. `PerformanceDeepEditBay.drawRackBayModule` has no title/Collapse header. In FX group mode the slot knobs (cols 1-3) are drawn with `MacroKnobWidget.draw(showLabel = false)` -- the `FxSlotCell` under each names the effect -- so an FX row's text band is `7 + max(captionH, FxSlotCell.HEIGHT)` instead of caption + cell (`fxTextBelowH`); focus mode keeps the captions (parameter names). Top header bar consolidates the tab strip (`[DECKS]` / `[MASTER]`), rack controls (`[Close Edit]`), persistent Learn indicator, and `[ALL]` dice into a single compact 26px line. Every row has the same shape: a title badge (`drawTitleBadge`) spanning both control lines, two stacked control lines, the knobs, and a right wing -- no row has a header band above its knobs. Deck badges are 54px with the deck tag in H1 (`A`/`B`/`BG`/`PV`; the `[SRC]`/`[FX]` pills show the mode). MASTER-tab badges are 90px in H2 (`MASTER`, `TRANS`, `WET/DRY`, `CLOCK`), and that tab's left wing is `masterRowW` = max(`deckRow1W`, 38% of grid width, ≤420px): Master = `[MIX]` + crossfader line over `[FX]` + chain header; Transitions = picker + queue nav (one line, vertically centered); FX Wet/Dry = badge only (Resync on the right); Clock = source/Link/BPM/beat dots over TAP/RESYNC/÷2/×2/±. Knobs dynamically scale between 24px and 100px (typically ~38-44px at compact half-view heights). Wing controls use `ctrlH` (21px in compact mode, 24px otherwise). Click-drag scrolling comes from a grid-wide `##perf_grid_drag_scroll` invisible button submitted first with `setNextItemAllowOverlap()`, plus the title-band drop-zone buttons; each calls `applyDragScroll()`, which scrolls the child by `io.mouseDelta.y` while that item is active. The background button also stops an empty-space drag from moving the host window. The knob body blends toward its accent color on hover (22%) and while dragging (38%).
...
- **Preferences Category Routing**: `PreferencesPanel` organizes application preferences into 9 clean categories (`GENERAL`, `SHADER_LOCATIONS`, `VIDEO_DISPLAY`, `AUDIO_ENGINE`, `TEMPO_SYNC`, `MIDI_CONTROLLER`, `OSC_CONTROLLER`, `SHORTCUTS`, `BROADCAST`) and supports targeted opening via `PreferencesPanel.open(category)`.
...
- **Audio Engine Quick Access**: Clicking the real-time BPM or DSP latency metric in the top performance stats bar (or opening `File > Preferences... > Audio Engine`, or pressing `Ctrl+P` / `Cmd+P`) navigates directly to the `AUDIO_ENGINE` category.
