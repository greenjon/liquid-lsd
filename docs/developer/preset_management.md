...
[`FXItemApplier.kt`](file:///home/gj/projects/liquid-lsd/src/main/kotlin/llm/slop/liquidlsd/presets/FXItemApplier.kt) resolves both single FX presets (`.lsdfx`) and 3-slot FX chains (`.lsdfxchain`) into a target deck's `assignedFxBank`, guaranteeing a reproducible FX state rather than arbitrary vacant-slot allocation. It is the single application path used by the FX browser, FX playlists, and both live FX queues.
**Chain scope**: both `.lsdfxchain` and `.lsdfx` items apply to **Chain 1 only** (`bank.applyFxChain(dto)`, the backward-compat single-arg delegate) — the live queue/playlist system is not chain-aware, by design; Chains 2/3 of a deck's bank are left for manual/persistent configuration outside the FXQ. This is a deliberate simplification, not a gap — revisit only if live-queue chain targeting is explicitly requested. (Contrast with `FXBrowserPanel.kt`'s manual "Load to Deck" context menus, which *do* let you target any of the 3 chains explicitly via `FxBank`'s indexed `applyFxSlot(chainIndex, slotIndex, ...)`/`applyFxChain(chainIndex, ...)` overloads.)
...
### `FXBrowserPanel.kt`
Unifies the formerly separate `FXPresetListPanel` (single `.lsdfx` presets) and `FXChainListPanel` (`.lsdfxchain` chains) into one Column 1 browser with ISF stock filters, saved singles, saved chains, and saved `.lsdfxbank` banks (`AssetType.FX_BANK`, `FileSystemManager.scanAllFxBanks()`), tier badges, and an All / Stock / Singles / Chains / Banks filter menu. Stock (built-in ISF) filters only support loading directly to a deck; saved presets and chains can additionally be added to FX playlists and live FX queues. Banks load only via `Load to FX1/FX2/MFX` (bank-scoped, not deck-scoped) and are not playlist/queue-eligible. "Load to Deck" submenus for stock/single/chain items nest a Chain 1/2/3 selector so any of a bank's 3 chains is reachable, not just Chain 1.
