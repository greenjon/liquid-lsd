...
[`FXItemApplier.kt`](file:///home/gj/projects/liquid-lsd/src/main/kotlin/llm/slop/liquidlsd/presets/FXItemApplier.kt) resolves both single FX presets (`.lsdfx`) and 3-slot FX chains (`.lsdfxchain`) into a target deck's dedicated insert `fxChain`, guaranteeing a reproducible FX state rather than arbitrary vacant-slot allocation. It is the single application path used by the FX browser, FX playlists, and both live FX queues.
...
### `FXBrowserPanel.kt`
Unifies the formerly separate `FXPresetListPanel` (single `.lsdfx` presets) and `FXChainListPanel` (`.lsdfxchain` chains) into one Column 1 browser with ISF stock filters, saved singles, saved chains, and saved `.lsdfxbank` banks (`AssetType.FX_BANK`, `FileSystemManager.scanAllFxBanks()`), tier badges, and an All / Stock / Singles / Chains / Banks filter menu. Stock (built-in ISF) filters and presets load directly to a deck's dedicated 3-slot insert FX chain (`deck.fxChain`); saved presets and chains can additionally be added to FX playlists and live FX queues. Banks load via `Load to FX1/FX2/MFX` (bank-scoped, not deck-scoped) and are not playlist/queue-eligible. "Load to Deck" submenus for stock/single items let you target any of the 3 slots in the deck's insert chain, while chains load directly.
