...
1. **Source Render & View Stage**:
   - **2D Sources (`!source.is3D`)**:
     ...
     - **3D Mode (`3D Mode >= 0.5`)**: The active 2D source renders into square `rawSourceFBO` (`height x height`). `tri_planar.vert` and `tri_planar.frag` (or `tetra_kaleido.frag`) project 3 or 6 orthogonal planes or 24-chamber kaleidoscope space folding (`Rotate X`, `Rotate Y`, `Rotate Z`, `Zoom`, `3D Persp`, `Separation`, `Depth Dim`) onto `cleanFBO`.
