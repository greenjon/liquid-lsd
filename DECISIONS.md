...
- **Decision**: Introduce multi-pass ISF shader processing, dual serialized FX slots per Deck, preset serialization, and a high-performance searchable shader picker:
  ...
  - **Bundled Creative ISF Shaders (`default_filters/`)**: Shipped default filters including `bloom.fs` (multi-pass bloom highlights and Gaussian diffusion), `feedback_trails.fs` (persistent motion trails), `glitch.fs` (scanline dislocation and chromatic aberration), and `mirror.fs` (coordinate space folding).
- **Rationale**:
