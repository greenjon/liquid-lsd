...
- **Decision**:
  - **Master row = `[MIX|FX]`**, built the same way as the deck rows: the crossfader header bar stays on top. The left wing stacks a `[MIX]` pill over an `[FX]` pill + Master FX `FxChainHeader`, and the right wing has the Master FX bypass. The mode is `PerformanceUiContext.isMasterRowFx` (`masterRowMode == "FX"` or Deep Edit's Mixer sub-tab is `FX`), mirroring `deckRowMode` / deck sub-tab. In FX mode the row's bankId becomes `MASTER_FX`, titled `MASTER (FX)`. Dropping an `.lsdfxchain` on the row title loads Master FX.
  - **Transitions keeps its own row.** Combining it with Master FX would only pair "things that aren't decks", and a 3-way Master toggle would be crowded. The crossfader/transition link is already covered by drops on the crossfader track.
  - **FX WET/DRY row kept.** It's the only Performance surface for the four per-deck send levels (`Deck X/FXChain/DryWet`), and 4 sends on one row maps well to hardware. Revisit if deck rows get an inline send control.
  - **Tabs**: ALL FX deleted. LIVE CONSOLE = Deck A, B, BG, Master. MASTER & FX = Master, Transitions, FX Wet/Dry. The saved tab index is clamped, so a saved ALL FX (index 3) opens LIVE CONSOLE.
  - **Removed** `focusedFxTarget`, the `"FX"` rack module id (a persisted `"FX"` disclosure is skipped on load), and `PerformanceFxControls` (the FX Wet/Dry wings moved to `PerformanceFxSendsControls`).
  - **Learn pinning**: `ParametersState.rackModuleBankIds` now maps each canonical module to every bank its row can show (`DECK_X` → `DECK_X`, `DECK_X_FX`; `MASTER` → `MASTER`, `MASTER_FX`, `TRANS`). Solo-mode auto-collapse therefore no longer closes a deck's or Master's Deep Edit while one of its FX knobs is learning. Before, only the removed `"FX"` module had this mapping.
- **Consequences**: Every FX chain is edited from the row of what it belongs to. MIDI/OSC mappings are unaffected because they target `Macro/<bank>/knob_N`, not tabs or rows.
- **Follow-up (same day): LIVE CONSOLE removed, Clock row + Global bank added.**
  - LIVE CONSOLE had become LIVE QUAD with Master in place of PV, so it was removed. The tabs are now **DECKS** (was LIVE QUAD) and **MASTER** (was MASTER & FX). A saved tab index that's out of range (a removed tab) loads as DECKS. The accepted cost: the on-screen crossfader no longer shares a screen with the decks (hardware mappings are unaffected).
  - MASTER's 4th row, **CLOCK & GLOBAL**:
    - **Header bar**: the tempo actions from `TempoSyncPanel` (clock source, Link status, BPM, beat dots, TAP with MIDI Learn, RESYNC, ÷2/×2, ±0.5). They're discrete actions, pressed mid-set, so they're buttons, not knobs.
    - **Not knobs**: Master BPM, because one bump drifts the whole show and it fights the tracker/Link. Input Gain, because it's set once per venue.
    - **Knobs**: a new canonical **`MacroEngine.GLOBAL`** bank. It's the only bank with no section scope (`MacroLearnState.sectionFor` = null), so one knob can span decks. It's canonical, so it's session-persisted and gets MIDI/OSC paths for free. It's never bundled into deck presets.
  - Column 3 MACROS gets a **GLB** tab. Pointing `activeTopTab` at a fake "Global" tab would confuse Deep Edit, which reads `activeTopTab`. Instead, `ParametersState.showGlobalMacros()` records the current top-tab/sub-tab key, and GLB stays shown only while that key is unchanged. Any other MACROS tab click or `navigateMacroPanelTo` clears it explicitly.
...
- **Context**: 2026-09-24. First step of making every major FX change (swap an effect, reorder, bypass a slot, load/save/new chain) possible from the Performance rows, with Deep Edit only for fine tuning (plan: `.planning/fx-performance-editing-plan.md`). Master FX was still an `FxBank` of 3 alternative chains, a leftover of the FX1/FX2/MFX bank era, reachable only through its own code paths and path prefix (`MFX/C{n}/...`). Grounding the plan turned up three live bugs:
  - The Performance panel synced FX knobs with labels that don't match any registered path (`"Deck A FX"`, and `"Master"` while Master's paths were `MFX/...`), so MST/deck **Resync** bound knobs to nothing and chain loads never refreshed knob labels.
  - Every async FX chain/slot load applied its result inside `CompletableFuture.thenAccept`, i.e. on the preset IO thread: disposing the replaced filters deleted GL framebuffers off the GL thread and swapped `FxChain.slots` while the renderer could be reading them.
  - Commit `cf73254` had re-embedded the FX chain into deck presets (`DeckPresetDto.fxChain`), reversing the 2026-09-19 decision below: loading a preset clobbered the deck's live FX, and any FX tweak marked the preset dirty.
- **Decision**:
  - Master FX is `Mixer.masterFxChain`, a plain `FxChain` exactly like each deck's, with paths under `Master/FX/...` (mirroring `Deck A/FX/...`) and rendered by the same `renderFxChainPass`. `FxBank`, `FXBankDto`, `.lsdfxbank`, bank scanning/clipboard/browser rows, the separate bank wet/dry blend, the chain 1/2/3 switcher and `ParametersTabs.drawFxBankGroupContent` are deleted. The two bank-only chains (*Subtle Optical Warmth*, *Digital Bitcrush Mosaic*) ship as ordinary `.lsdfxchain` files; `Mixer.loadDefaultFxChains()` seeds all five chains from named chain files.
  - FX chains are session state only: `DeckPresetDto.fxChain` is removed again, and `MixerDto` persists `masterFxChain` plus `deckA/B/BG/PVFxChain`. No legacy-session shims (beta): an old session's Master FX and deck FX fall back to the starter chains once.
  - `FxMacroSync` has one code path: `syncChain(bankId, label, chain)` writing `"$label/FX/..."`, with `labelFor`/`chainFor`/`bankIdFor`/`syncFor`/`syncAll` resolving everything from the FX bank id. It replaces the 8 copied `when (bankId)` blocks in `PerformanceMatrixPanel`, MacroPanel's hard-coded chain 0, and FXChainMacroStrip's stale `/C<n>` parsing. Session restore now runs a non-forced `syncAll`, so knob labels match the restored chains while hand-retargeted knobs stay untouched.
  - New `presets/FxOps.kt` is the single mutation path for any FX chain (load/apply/clear chain, set/load/apply/clear slot, playlist/queue `applyItem`). Every op is queued and applied in `FxOps.drainOnGlThread`, once per frame next to `PresetManager.applyPendingPresets`, then re-syncs that chain's knobs. `FXItemApplier` is folded into it. The FX browser can now also load into and save from Master FX.
  - FX-row MIDI learn defaults (`fxMacroSyncBankIds`) now cover all five FX banks, not just Master's.
- **Rationale**: One FX model means the Step 2/3 slot cells and chain header get written once for all five rows. Routing every change through `FxOps` fixes the threading bug by construction and gives the upcoming fade-on-swap and dirty tracking a single place to hook in. Keeping FX out of presets preserves the "FX is independent of what's playing" model the FX queues and playlists rely on.
...
- **Decision**:
  - **Source picker**: the Performance deck-row generator badge is now a button opening the same picker as the Parameters title bar. Both, plus the launchpad, go through `DeckSourcePicker` so every entry point uses `changeVisualSourceSafely`.
  - **Empty-deck launchpad in Deep Edit**: moved from `ParametersPanel` to `DeckSourcePicker.drawLaunchpad`. Its Add Source button now opens the shader picker (which lists external video) instead of a plain menu of built-in sources.
  - **Shortcuts**: Deep Edit swaps its own `rackSelectedCell` into `ParametersState` only while it draws, so a global handler would have edited Classic's hidden selection. `PerformanceMatrixPanel` instead runs `ParametersKeyboard` inside the owning Deep Edit (last clicked, else first open) for save + copy/paste/clear, and at top level for undo only when no Deep Edit is open. Cell edits also require the Performance window focus, so Delete in the Library can't reset a parameter. Save is never global in Performance Mode because `handleSaveDeck` silently overwrites and `activeTopTab` there is not a visible choice.
  - **Macro go-to-target**: `MacroBindingInspector.navTargetFor` maps a parameter path to (module, top tab, sub-tab) — including Deck FX, Mixer TRANS and MFX, which previously all landed on SRC/CTRL — and in Performance Mode opens that Deep Edit with the cell selected. Learn on a Performance knob opens the row's Deep Edit if collapsed.
  - **Slider hover flag**: `CustomRangeSlider.isAnySliderHovered` is now reset every frame in both modes; it was only reset in Classic, which left mouse-wheel scrolling permanently off in Deep Edit's Properties after the first slider hover.
  - **Mixer width**: in Performance Mode the Mixer column is capped by `PerformanceMatrixPanel.calculateMinWidth` (Deep Edit side rail + grid + 450px Properties allowance) instead of Classic's `ParametersPanel.calculateRequiredWidth`, which varied with the hidden Parameters tab.
- **Rationale**: Every remaining reason to press F4 mid-set is removed before Classic itself is, so the removal can't strand a feature.
