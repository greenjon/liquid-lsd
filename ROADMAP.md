...
| **Live Performance Console & FX Macro Integration** | `ui/PerformanceMatrixPanel.kt`, `macro/FxMacroSync.kt`, `rendering/FxBank.kt`, `rendering/Renderer.kt` | **v1.1** | **PLANNING** | New `LIVE CONSOLE` tab unifying Deck A/B + active FX bank/chain (FX1/FX2/MFX-switchable) + Master/Transitions into one 4×4 surface with smart-default knob binding. Requires `FxBank` to first be simplified from "3 chains in series" to "3 alternative chains, one active at a time" (see Milestone 1 deferred notes). |
